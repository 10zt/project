	...
	<el-main class="bg edit_wrap">
		...
		<el-row class="row_ce"> 
				...
				<el-form-item label="学生用户" prop="student_users">
							...
							<!--v-if="user_group === '管理员' || (form['learning_courses_id'] && $check_field('set','student_users')) || (!form['learning_courses_id'] && $check_field('add','student_users'))" :disabled="disabledObj['student_users_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','student_users')">{{form['student_users']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['learning_courses_id'] && $check_field('set','student_users')) || (!form['learning_courses_id'] && $check_field('add','student_users'))" id="student_users" v-model="form['student_users']" :disabled="disabledObj['student_users_isDisabled']">
				...
				<el-form-item label="学生姓名" prop="student_name">
							  ...
							  v-if="user_group === '管理员' || (form['learning_courses_id'] && $check_field('set','student_name')) || (!form['learning_courses_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']"></el-input>
				...
				<el-form-item label="课程名称" prop="course_name">
							  ...
							  v-if="user_group === '管理员' || (form['learning_courses_id'] && $check_field('set','course_name')) || (!form['learning_courses_id'] && $check_field('add','course_name'))" :disabled="disabledObj['course_name_isDisabled']"></el-input>
				...
				<el-form-item label="课程日期" prop="course_date">
												<el-input id="course_date" v-model="form['course_date']" placeholder="请输入课程日期"
							  v-if="user_group === '管理员' || (form['learning_courses_id'] && $check_field('set','course_date')) || (!form['learning_courses_id'] && $check_field('add','course_date'))" :disabled="disabledObj['course_date_isDisabled']"></el-input>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/learning_courses/view','set') || $check_action('/learning_courses/view','add') || $check_option('/learning_courses/table','examine')">
	...
	export default {
		...
		data() {
			return {
				...
				url_upload: "~/api/learning_courses/upload?",
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/learning_courses/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/learning_courses/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/learning_courses/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/learning_courses/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/learning_courses/view','get');
