		...
		<el-row class="row_ce"> 
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="用户姓名" prop="user_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['class_teacher_id'] && $check_field('set','user_name')) || (!form_sub['class_teacher_id'] && $check_field('add','user_name'))"></el-input>
				...
				<el-form-item label="用户性别" prop="user_gender">
						...
						v-if="user_group === '管理员' || (form_sub['class_teacher_id'] && $check_field('set','user_gender')) || (!form_sub['class_teacher_id'] && $check_field('add','user_gender'))">
				...
				<el-form-item label="联系电话" prop="contact_phone_number">
								<el-input id="contact_phone_number" v-model="form_sub['contact_phone_number']" placeholder="请输入联系电话" type="tel"
						v-if="user_group === '管理员' || (form_sub['class_teacher_id'] && $check_field('set','contact_phone_number')) || (!form_sub['class_teacher_id'] && $check_field('add','contact_phone_number'))">
					</el-input>
					<div v-else-if="$check_field('get','contact_phone_number')">{{form_sub['contact_phone_number']}}</div>
							</el-form-item>
			</el-col>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/class_teacher/view','set') || $check_action('/class_teacher/view','add') || $check_option('/class_teacher/table','examine')">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async submit_check(param){
				...
				var form_sub = Object.assign({} ,this.form_sub);
																																								let contact_phone_number_phone_regular = /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/;
				if(form_sub.contact_phone_number && !contact_phone_number_phone_regular.test(form_sub.contact_phone_number)){
					return "手机号格式错误"
				}
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["class_teacher_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?class_teacher_id=" + form_sub["class_teacher_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/class_teacher/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/class_teacher/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/class_teacher/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/class_teacher/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/class_teacher/view','get');
